...
│   └── meta_backtest.py      <-- new: wires the trained model into app.py
└── app.py                     <-- REPLACE (ticker import + direction + suggest_eps + meta-filter UI)
```
...
**Now wired into `app.py`:** a "Meta-Model Filter" section in the sidebar
(checkbox + probability threshold slider) appears automatically once
`models/meta_model.pkl` exists. When enabled, Tab 3 shows how many
historical entries the model kept for the inspected pair and backtests
only those; Tab 4 swaps in a meta-filtered version of the summary table
for every pair. New file `src/meta_backtest.py` does this without
modifying `signals.py`, `backtest.py`, or `meta_labeling.py` — it's a
thin orchestration layer on top of all three.

Verified: filtering never *adds* exposure (filtered position count is
always ≤ unfiltered), vetoed trades measurably change the position
series, and both the single-pair and multi-pair filtered paths return
the exact same shape as their unfiltered counterparts for drop-in
compatibility.
